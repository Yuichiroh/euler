...
object P65 extends App {
  ...
  val a = 2 #:: Stream.from(2, 2).flatMap(Seq(1, _, 1))
  val p = seq(1, a(0), a.tail)
  //  val q = seq(0, 1, a.tail)

  def solution = p(100).toString.toCharArray().map(_.toString.toInt).sum
